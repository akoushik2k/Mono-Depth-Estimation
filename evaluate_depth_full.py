...
from networks.RTMonoDepth.RTMonoDepth import DepthDecoder, MobileNetV2Encoder
...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        # print("Keys in encoder.pth:")
        # print(encoder_dict.keys())
        
        dataset = datasets.KITTIRAWDataset(opt.data_path, filenames, encoder_dict['height'], encoder_dict['width'], [0], 4, is_train=False, use_depth_hints=False)
        dataloader = DataLoader(dataset, 16, shuffle=False, num_workers=opt.num_workers, pin_memory=True, drop_last=False)

        encoder = MobileNetV2Encoder(pretrained=False).cuda().eval()        
        depth_decoder = DepthDecoder(encoder.num_ch_enc).cuda().eval()
