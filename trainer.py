...
from networks.RTMonoDepth.RTMonoDepth import DepthDecoder, DepthEncoder, MobileNetV2Encoder
...
class Trainer:
    def __init__(self, options):
        ...
        if self.opt.use_learnable_K:
            focal_len = 807.1375
            self.K = torch.tensor([focal_len/self.opt.width, focal_len/self.opt.height, 0.5, 0.5], dtype=torch.float32).to(self.device).requires_grad_()
            self.parameters_to_train = [self.K]
        ...
        self.models["encoder"] = MobileNetV2Encoder(pretrained=True)
        self.models["encoder"].to(self.device)
        self.parameters_to_train += list(self.models["encoder"].parameters())
    ...
    def load_model(self):
        ...
        for k in model_dict:
            if k not in pretrained_dict:
                print(f"Skipped {k} (not found in pretrained)")
        # loading adam state
